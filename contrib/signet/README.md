...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<blackcoin-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<blackcoin-cli args>]`

* `--cmd` lets you customize the blackcoin-cli path. By default it will look for it in the PATH
...
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `blackcoin-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    CLI="./blackcoin-cli -conf=mysignet.conf"
