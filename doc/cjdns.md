# CJDNS support in Blackcoin Core

It is possible to run Blackcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Blackcoin Core, similarly to a VPN (either in the host/OS or on
the network router).
...
redundancy and robustness for both the Blackcoin network and individual nodes.
...
## Run Blackcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Blackcoin Core
...
When enabled, this option tells Blackcoin Core that it is running in an
...
IPv6 local network. This helps Blackcoin Core perform better address management:
...
CJDNS support was added to Blackcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `blackcoin-cli -addrinfo` to see the
...
## CJDNS-related information in Blackcoin Core

There are several ways to see your CJDNS address in Blackcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `blackcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `blackcoin-cli getpeerinfo`).
