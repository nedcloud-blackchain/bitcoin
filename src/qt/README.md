This directory contains the source code for the Blackcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Blackcoin UI.
...
- ClientModel is used by the main application `blackcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a blackcoin:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: BLK / mBLK / etc. handling
* `callback.h`
...
4. Enter `blackcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `blackcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `blackcoin-qt`
9. You're all set! Start developing, building, and debugging the Blackcoin Core GUI
