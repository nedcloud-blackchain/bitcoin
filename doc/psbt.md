# PSBT Howto for Blackcoin Core

Since Blackcoin Core 0.17, an RPC interface exists for Partially Signed Blackcoin
...
PSBT is an interchange format for Blackcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Blackcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Blackcoin transaction (in network format)
...
#### Multisig with multiple Blackcoin Core instances
...
Blackcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `blackcoin-cli` is used.
...
Later, when *V* BLK has been received on *Amulti*, and Bob and Carol want to
