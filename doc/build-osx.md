...
This guide describes how to build blackcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Blackcoin Core from source.
...
Now that all the required dependencies are installed, let's clone the Blackcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `blackcoind` or  `blackcoin-qt`.
...
Blackcoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Blackcoin Core application using `make deploy`.
...
## Building Blackcoin Core

### 1. Configuration

There are many ways to configure Blackcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Blackcoin Core:
...
## Running Blackcoin Core

Blackcoin Core should now be available at `./src/blackcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/blackcoin-qt`.

The first time you run `blackcoind` or `blackcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/blackcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/blackcoin"

touch "/Users/${USER}/Library/Application Support/blackcoin/blackcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/blackcoin/blackcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/blackcoin/debug.log
```

## Other commands:

```shell
./src/blackcoind -daemon      # Starts the blackcoin daemon.
./src/blackcoin-cli --help    # Outputs a list of command-line options.
./src/blackcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/blackcoin-qt -server # Starts the blackcoin-qt server mode, allows blackcoin-cli control
```
