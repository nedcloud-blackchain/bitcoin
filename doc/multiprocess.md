# Multiprocess Blackcoin

On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `blackcoin-node`, `blackcoin-wallet`, and `blackcoin-gui` executables alongside existing `blackcoind` and `blackcoin-qt` executables.

`blackcoin-node` is a drop-in replacement for `blackcoind`, and `blackcoin-gui` is a drop-in replacement for `blackcoin-qt`, and there are no differences in use or external behavior between the new and old executables. But internally (after [#10102](https://github.com/bitcoin/bitcoin/pull/10102)), `blackcoin-gui` will spawn a `blackcoin-node` process to run P2P and RPC code, communicating with it across a socket pair, and `blackcoin-node` will spawn `blackcoin-wallet` to run wallet code, also communicating over a socket pair. This will let node, wallet, and GUI code run in separate address spaces for better isolation, and allow future improvements like being able to start and stop components independently on different machines and environments.

## Next steps

Specific next steps after [#10102](https://github.com/bitcoin/bitcoin/pull/10102) will be:

- [ ] Adding `-ipcbind` and `-ipcconnect` options to `blackcoin-node`, `blackcoin-wallet`, and `blackcoin-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `blackcoin-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
make
src/blackcoin-node -regtest -printtoconsole -debug=ipc
BITCOIND=blackcoin-node test/functional/test_runner.py
```
