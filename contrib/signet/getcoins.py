...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing blackcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='blackcoin-cli', help='blackcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Blackcoin address to which the faucet should send')
...
parser.add_argument('blackcoin_cli_args', nargs='*', help='Arguments to pass on to blackcoin-cli (default: -signet)')

args = parser.parse_args()

if args.blackcoin_cli_args == []:
    args.blackcoin_cli_args = ['-signet']


def blackcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.blackcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = blackcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = blackcoin_cli(['getnewaddress', 'faucet', 'bech32'])
