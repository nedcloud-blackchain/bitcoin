...
Below are some notes on how to build Blackcoin Core for Windows.

The options known to work for building Blackcoin Core on Windows are:
...
Note that for WSL the Blackcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/blackcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\blackcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/blackcoin
