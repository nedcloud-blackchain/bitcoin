...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " blackcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  blackcoin-tx [options] <hex-tx> [commands]  Update hex-encoded blackcoin transaction\n"
                "or:     blackcoin-tx [options] -create [commands]   Create hex-encoded blackcoin transaction\n"
                "\n";
