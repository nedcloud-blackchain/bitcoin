...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 BLK) in its wallet. Using them is much more
...
the Blackcoin Core node application logic. For custom behaviour, subclass the
