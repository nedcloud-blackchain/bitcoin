...
dnl Output: If qt version is auto, set blackcoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$blackcoin_qt_want_version" = "auto" && test "$blackcoin_qt_force" != "yes"; then
    if test "$blackcoin_enable_qt" != "no"; then
      AC_MSG_WARN([$1; blackcoin-qt frontend will not be built])
    fi
    blackcoin_enable_qt=no
    blackcoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$blackcoin_enable_qt" != "no" && test "$blackcoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build blackcoin-qt GUI (default=auto)])],
    [
     blackcoin_qt_want_version=$withval
     if test "$blackcoin_qt_want_version" = "yes"; then
       blackcoin_qt_force=yes
       blackcoin_qt_want_version=auto
     fi
    ],
    [blackcoin_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); blackcoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: blackcoin_enable_qt, blackcoin_enable_qt_dbus, blackcoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$blackcoin_cv_static_qt" = "yes"; then
  ...
  BITCOIN_QT_CHECK([
    blackcoin_enable_qt=yes
    blackcoin_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      blackcoin_enable_qt_test=no
    fi
    blackcoin_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      blackcoin_enable_qt_dbus=yes
    fi
  ...
  if test $blackcoin_enable_qt = "yes"; then
    AC_MSG_RESULT([$blackcoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$blackcoin_enable_qt])
  fi
...
dnl Output: blackcoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, blackcoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [blackcoin_cv_static_qt=yes],
      [blackcoin_cv_static_qt=no])
    ])
])
