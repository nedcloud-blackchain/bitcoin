...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //blackcoin-tx using stdin
        std::string val;
...
{
    // Windows: C:\Users\Username\AppData\Roaming\blackcoin
    // macOS: ~/Library/Application Support/blackcoin
    // Unix-like: ~/.blackcoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/blackcoin";
#else
    // Unix-like
