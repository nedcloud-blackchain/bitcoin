...
static RPCHelpMan verifymessage()
{
    return RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The blackcoin address to use for the signature."},
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "blackcoin-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new blackcoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new blackcoin-echo executable,
                // and spawn blackcoin-echo below instead of blackcoin-node. But
                // using blackcoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("blackcoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a blackcoind
                // process not a blackcoind-node process, so just create a local
