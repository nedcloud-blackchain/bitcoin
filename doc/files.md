...
The data directory is the default location where the Blackcoin Core files are stored.
...
3. All content of the data directory, except for `blackcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Blackcoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `blackcoin.conf`        | User-defined [configuration settings](blackcoin-conf.md) for `blackcoind` or `blackcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `blackcoind.pid`        | Stores the process ID (PID) of `blackcoind` or `blackcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `blackcoind` or `blackcoin-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [blackcoin.conf](blackcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`blackcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Blackcoin Core:
