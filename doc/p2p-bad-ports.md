When Blackcoin Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Blackcoin node is listening,
or one where a service is listening that is not related to the Blackcoin network.
As a result, this service may occasionally get connection attempts from Blackcoin
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Blackcoin Core,
trying to connect because it thinks there is a Blackcoin node on that
...
Below is a list of "bad" ports which Blackcoin Core avoids when choosing a peer to
